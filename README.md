...
Pour lancer le projet, assurez-vous d'avoir les éléments suivants:

- docker installé, [téléchargeable
  ici](https://docs.docker.com/desktop/setup/install/windows-install/)
- Un navigateur web installé (Chrome, Firefox, Edge...)
...
1. Ouvrir le terminal dans le dossier de destination désiré du repo.
2. Lancer la commande suivante qui va cloner le code dans ce même dossier.
   ```bash
   git clone "https://github.com/OpenClassrooms-Student-Center/724-events-testeur-logiciel.git"
   ```
3. Une fois le repo téléchargé, lancer cette fois-ci la commande docker suivante, toujours dans le même dossier:

```bash
docker compose up --build
```

4. Attendre une petite minute ou deux puis se rendre sur le [site web](http://localhost:3000/) de 724events.

## Lancement des tests

Pour lancer les tests, assurez-vous d'avoir les éléments suivants:
...
Pour en lancer un, cliquer sur le test souhaité ou lancer la commande `npx cypress run` pour tous les lancer.

## Génération du rapport

Pour générer le rapport des tests:

1. Lancer la commande suivante pour initier le projet NodeJS si ce n'est pas déja fait (laisser tout par défaut) : `npm init`
